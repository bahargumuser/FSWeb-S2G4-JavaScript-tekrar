...
/* 	GÖREV 1:  
		- CemberinCevresi fonksiyonunu kullanarak aşağıdaki yönergeleri uygulayın:
			...
			2. Global değişken olarak verilmiş pi sayısı fonksiyon içinde kullanılacak
			(pi sayısı fonksiyonun parametresi olarak alınmayacaktır)
...
/* 	GÖREV 2:  
		- CemberinAlani fonksiyonunu kullanarak aşağıdaki yönergeleri uygulayın:
			...
			3. Çemberin alanı hesaplanacaktır 
			(💡 İPUCU: Çemberin alanı = pi * yarıçapın karesi, yarıçapın karesini bulmak için Javascript içinde tanımlı Math kütüphanesini kullanabilirsiniz. 
				Math.pow(yaricap,2))
			4. Hesaplanan çemberin alanı döndürülecektir.
		*/
		
		const pi = 3.14
		function CemberinAlani(cemberinyaricapi){
		const alan = pi * Math.pow(cemberinyaricapi,2)
		return alan 
		}
...
/* 	GÖREV 3:
		...
		Bu diziyi kullanarak aşağıdakileri uygulayın:
			...
			3c. `ucetambolunenler` dizisindeki sayıların toplamını .reduce metoduyla bulup,
			 sonucu `ucebolunenlerintoplami` değişkenine yazdırın 
			(.reduce metodunu kullanın)
			3d. `besyuzdenkucuksayilar` adında bir dizi oluşturarak, 
			sayilar dizisinin içindeki 500'den küçük sayıları bu diziye atayın 
			...
			sayilar dizisi içerisinde birden fazla kez yazılmış sayıları tespit ederek
...
/*  (oto test yok) sayilar dizisi içinde kaç adet sayı olduğunu konsola yazdırın */
	...
	// 3b çözümü:
		...
		sayilar.forEach(testedici)
		function testedici(sayi) {
		if (sayi%3 == 0){
	...
	let ucebolunenlerintoplami = ucetambolunenler.reduce(function (previousValue, currentValue) {
	return previousValue + currentValue;
	});

	console.log("toplam = "+ ucebolunenlerintoplami)
